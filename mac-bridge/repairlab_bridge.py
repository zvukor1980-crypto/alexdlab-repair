...
SITE = "https://zvukor1980-crypto.github.io"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def headers_json(self, code=200):
        ...
        self.end_headers()
    def allowed(self):
        return self.headers.get("Origin") == SITE and self.headers.get("X-RepairLab-Token") == TOKEN
    def send_json(self, value, code=200):
        self.headers_json(code); self.wfile.write(json.dumps(value, ensure_ascii=False).encode())
    def do_OPTIONS(self):
        if self.headers.get("Origin") != SITE: return self.send_json({"error":"origin"},403)
        self.send_response(204); self.send_header("Access-Control-Allow-Origin",SITE)
        self.send_header("Access-Control-Allow-Headers","Content-Type, X-RepairLab-Token")
...
if __name__ == "__main__":
    ...
    url=f"https://zvukor1980-crypto.github.io/alexdlab-repair/#bridge={PORT}&token={TOKEN}"
    webbrowser.open(url)
